    ...
    <div className="space-y-3 text-xs sm:text-sm">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex flex-wrap gap-2 sm:gap-2 overflow-x-auto">
            {product.colors.map((color) => (
              <button 
                key={color.id} 
                className={`w-10 h-10 rounded-md border-2 flex items-center justify-center relative overflow-hidden ${
              ...
              >
                ...
                {selectedColor === color.id && (
                  // Small check badge in the top-right instead of darkening the whole image
                  <div className="absolute top-1 right-1 w-6 h-6 rounded-full bg-white/95 flex items-center justify-center shadow">
      ...
      {(priceData?.bulk_price?.length > 0 || product?.bulkPricing?.length > 0) && stock > 0 && (
        <div className="text-xs sm:text-sm font-medium text-orange-600">
         buy in bulk? <a href="#" className="underline font-medium"> bulk pricing options</a>
        </div>
      )}
    </div>
  );
